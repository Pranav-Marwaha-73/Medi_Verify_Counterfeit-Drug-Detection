...
import { Upload, Camera, X, Edit3 } from "lucide-react";
...
const ScanSection = ({ onVerificationComplete, setIsLoading, showAlert }) => {
  ...
  const verifyManualId = async () => {
    if (!manualId.trim()) {
      showAlert("Please enter a medicine ID", "error");
      return;
    }
    
    setIsLoading(true);
    try {
      const med = await getMedicine(manualId.trim());
      console.log("[ScanSection] manual med:", med);
      ...
      if (!med.found) {
        ...
        onVerificationComplete(counterfeitResult);
        showAlert(`Medicine with ID ${manualId.trim()} not found in database - Marked as Counterfeit`, "error");
        return;
      }
        ...
        <div className="scan-area">
          {inputMode === "scan" ? (
            ...
            <div className="manual-input-zone">
              <div className="manual-input-content">
                <div className="manual-icon"><Edit3 size={48} /></div>
                <h3>Enter Medicine ID Manually</h3>
                <input
                  type="text"
                  value={manualId}
                  onChange={(e) => setManualId(e.target.value)}
                  placeholder="Enter barcode/QR code number (e.g., 8904045410416)"
                  className="manual-input-field"
                  onKeyPress={(e) => e.key === 'Enter' && verifyManualId()}
                />
                <p className="manual-hint">Enter the numeric code from the medicine package</p>
              </div>
            </div>
          )}
          ...
          <div className="scan-controls">
            <button 
              ...
              onClick={inputMode === "scan" ? scanImage : verifyManualId}
            >
              <Upload size={18} /> 
              {inputMode === "scan" ? "Scan & Verify" : "Verify ID"}
            </button>
