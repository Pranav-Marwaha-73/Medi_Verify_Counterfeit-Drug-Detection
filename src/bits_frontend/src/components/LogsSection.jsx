...
const LogsSection = ({ transactionLogs = [], showAlert }) => {
  ...
  const exportLogs = () => {
    ...
    const csvContent = [
      ['Timestamp', 'Medicine ID', 'Medicine Name', 'Status', 'Manufacturer', 'Batch Number', 'Manufacturing Date', 'Expiry Date', 'Blockchain Hash', 'User ID', 'Full Principal ID'],
      ...transactionLogs.map(log => [
        ...
        log?.verificationHash ?? log?.blockchainHash ?? 'N/A',
        log?.user ?? 'Unknown',
        log?.userPrincipal ?? 'N/A'
      ])
